...
def test_empty_input_returns_empty_list() -> None:
    assert KnowledgeExtractor().extract([]) == []


def test_single_fragment_maps_to_single_knowledge() -> None:
    result = KnowledgeExtractor().extract([_fragment("frag-1", 0, "Hello")])
    assert len(result) == 1
    knowledge = result[0]
...
def test_multiple_fragments_preserve_order() -> None:
    ...
    result = KnowledgeExtractor().extract(fragments)
...
def test_knowledge_id_is_stable_and_matches_algorithm() -> None:
    ...
    first = KnowledgeExtractor().extract(fragments)
    second = KnowledgeExtractor().extract(fragments)
...
def test_extra_field_is_forbidden() -> None:
    with pytest.raises(ValidationError):
        Knowledge(knowledge_id="k", fragment_id="f", text="t", extra_field="nope")
